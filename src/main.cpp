...
#if defined(NDEBUG)
# error "H2O Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
        return 385000 * COIN; // premine 0.5%
    }
    
    if (nPrevHeight < 2160) {
        nSubsidy = 0.05 * COIN; // launch period, market establishment
    }else if (nPrevHeight < 4320){
        nSubsidy = 10 * COIN; // Step 1
    }else if (nPrevHeight < 5040){
        nSubsidy = 20 * COIN; // Step 2
    }else if (nPrevHeight < 5760){
        nSubsidy = 30 * COIN; // Step 3
    }else if (nPrevHeight < 6484){
        nSubsidy = 40 * COIN; // Step 4
    }else if (nPrevHeight < 7200){
        nSubsidy = 60 * COIN; // Step 5
    }else if (nPrevHeight < 7920){
        nSubsidy = 80 * COIN; // Step 6
    }else if (nPrevHeight < 8640){
        nSubsidy = 120 * COIN; // Step 7
    }else if (nPrevHeight < 9360){
    ...
    // subsidy halving on the half year basis resulting in 76.4 million H2O in 2030
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // H2O : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(H2O): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(H2O): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // H2O : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(H2O): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(H2O): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        H2O Related Inventory Messages
