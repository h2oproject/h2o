...
const char * const BITCOIN_CONF_FILENAME = "h2o.conf";
const char * const BITCOIN_PID_FILENAME = "h2od.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "h2o" is a composite category enabling all H2O-related debug output
            if(ptrCategory->count(string("h2o"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\H2OCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\H2OCore
    // Mac: ~/Library/Application Support/H2OCore
    // Unix: ~/.h2ocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "H2OCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/H2OCore";
#else
    // Unix
    return pathRet / ".h2ocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty h2o.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override h2o.conf
