...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "H2ONetwork") type = H2ONetwork;
    if(strType == "H2ONetworkVariable") type = H2ONetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == H2ONetwork) s = "H2ONetwork";
    if(type == H2ONetworkVariable) s = "H2ONetworkVariable";
