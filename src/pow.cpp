...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    /* current difficulty formula, h2o - DarkGravity v3, written by Evan Duffield - evan@h2o.org */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // for mainnet/regtest DGW is activated at height 200, previous block are retargeted using standard BTC retarget algo
    if (Params().NetworkIDString() == CBaseChainParams::MAIN || Params().NetworkIDString() == CBaseChainParams::REGTEST) {
        if (pindexLast->nHeight + 1 >= 200) retarget = DIFF_DGW;
        else retarget = DIFF_BTC;
    // for testnet DGW is activated at height 500, previous block are retargeted using standard BTC retarget algo
    } else {
