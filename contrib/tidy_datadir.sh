...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete H2O database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no H2O datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old H2O datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected H2O 0.7 datadir."
    ;;
  3)
    echo "Detected H2O pre-0.8 datadir."
    ;;
  4)
    echo "Detected H2O 0.8 datadir."
    ;;
esac
