...
*Setup instructions for a Gitian build of H2O Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the H2O
...
- [Building H2O Core](#building-h2o-core)
...
Clone the git repositories for H2O Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/h2ocoin/h2o
```
...
in the H2O Core repository under 'Fetch and build inputs' to install sources which require
...
To build H2O Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the H2O Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/h2o/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/h2ocoin/h2o
...
For example:
```bash
URL=https://github.com/crowning-/h2o.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit h2o=${COMMIT} --url h2o=${URL} ../h2o/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit h2o=${COMMIT} --url h2o=${URL} ../h2o/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit h2o=${COMMIT} --url h2o=${URL} ../h2o/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the h2o git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../h2o/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/h2o.git
SIGPATH=/some/root/path/h2o-detached-sigs.git

./bin/gbuild --url h2o=${BTCPATH},signature=${SIGPATH} ../h2o/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/h2o-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/h2o-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/h2o-osx-build.assert
```
...
[h2o/gitian.sigs](https://github.com/h2ocoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
